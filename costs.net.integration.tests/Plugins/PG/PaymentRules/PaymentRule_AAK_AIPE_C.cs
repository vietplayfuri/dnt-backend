namespace costs.net.integration.tests.Plugins.PG.PaymentRules
{
    ...
    public class PaymentRule_AAK_AIPE_C : PaymentRuleIntegrationTestBase
    {
        // rule name - "AAK-VideoPostProduction-AIPE-NonDPV-C"
        [Test]
        public async Task PaymentRule_AIPE_AAK_VPP_C_AIPE_stage()
        {
            // Arrange
            InitData(
                isAipe: true,
        ...
        [Test]
        public async Task PaymentRule_AIPE_AAK_VPP_C_OE_stage()
        {
            // Arrange
            InitData(
                isAipe: true,
                ...
                items: new List<CostLineItemView>() { new CostLineItemView() { Name = Constants.CostSection.InsuranceTotal, ValueInDefaultCurrency = 20002 } },
                contentType: Constants.ContentType.Video,
                productionType: Constants.ProductionType.CgiAnimation
            );
            ...
            // Assert
            // we should have 0% of the cost total
            receipt.PaymentAmount.ShouldBeEquivalentTo(0);
        }

        [Test]
        public async Task PaymentRule_AIPE_AAK_VPP_C_FA_stage_with_previous_payments()
        {
            // Arrange
            InitData(
                isAipe: true,
                ...
                {
                    new CostStageRevisionPaymentTotal() { LineItemTotalType = Constants.CostSection.CostTotal, LineItemTotalCalculatedValue = 3000 }
                },
                contentType: Constants.ContentType.Video,
                productionType: Constants.ProductionType.CgiAnimation
            );
            ...
            // Assert
            // 100% of remaining money - 17400
            receipt.PaymentAmount.ShouldBeEquivalentTo(17400);
        }

        [Test]
        public async Task PaymentRule_AIPE_AAK_VPP_C_FA_stage_with_previous_payments_with_overpayment()
        {
            // Arrange
            InitData(
                isAipe: true,
                ...
                payments: new List<CostStageRevisionPaymentTotal>()
                {
                    new CostStageRevisionPaymentTotal() { LineItemTotalType = Constants.CostSection.CostTotal, LineItemTotalCalculatedValue = 26000 }
                },
                contentType: Constants.ContentType.Video,
